...
class Statistics:
    ...
    async def workers_Assignments(self) -> dict:
        ...
        try:
            ...
            count_ass = [
                {a.person.name_first + " " + a.person.name_last: len(a.assignments)}
                for a in data_wa
            ]
            ...
            average_assignments = (
                statistics.mean(avg_ass)
                if avg_ass
                else 0  # Prevent division by zero if the list is empty
            )
            dict_stat["avgCountAssigment"] = round(average_assignments)

            max_assignments = round(
                max(avg_ass, default=0), 1
            )  # Max tasks assigned to a worker
            min_assignments = round(
                min(avg_ass, default=0), 1
            )  # Min tasks assigned to a worker

            # Identify over-tasked and under-utilized workers
            over_tasked_workers = [
                worker.person.name_first + " " + worker.person.name_last
                for worker in data_wa
                if len(worker.assignments) > average_assignments
            ]
            under_utilized_workers = list(
                {
                    worker.person.name_first
                    for worker in data_wa
        ...
        try:
            ...
            for a in data_wa:
                for article in a.articles:
                    article_counter[article.description] = (
                        article_counter.get(article.description, 0) + 1
                    )

            # Sort articles by frequency and get top results
            sorted_articles = sorted(
                article_counter.items(), key=lambda x: x[1], reverse=True
            )
            ...
            for a in data_wa:
                ...
                company_wise_purchase[company] = company_wise_purchase.get(
                    company, 0
                ) + len(a.articles)

            # Get top companies
            sorted_companies = sorted(
                company_wise_purchase.items(), key=lambda x: x[1], reverse=True
            )
            dict_stat["Top_Companies"] = sorted_companies[:10]  # Top 10 companies

            prices = [
                article.purchase_price
                for person in data_wa
                for article in person.articles
                if article.purchase_price is not None
            ]
